...
from ..schemas import PaymentLinkRequest, StoreSleepRequest, StoreStatus

router = APIRouter(tags=["store"])


class StoreStatusBroadcaster:
    """Класс для рассылки обновлений статуса магазина через SSE."""

    def __init__(self):
        """Инициализирует broadcaster."""
    ...
    async def broadcast(self, payload: dict):
        """Рассылает обновление всем зарегистрированным слушателям."""
        ...
        if updated_at is not None:
            if isinstance(updated_at, str):
                try:
                    ...
                    logger.warning(
                        f"Не удалось преобразовать updated_at из строки: {updated_at}, ошибка: {e}, используем текущее время"
                    )
...
async def stream_store_status(
    ...
    """Стримит обновления статуса магазина через SSE."""
    import logging
...
async def _ensure_awake_if_needed(db: Optional[AsyncIOMotorDatabase], doc: dict):
    ...
    Если указано время sleep_until и оно уже прошло, магазин автоматически
    выводится из режима сна.
    """
    ...
    try:
        ...
        if is_sleep_mode and sleep_until:
            ...
            if sleep_until_dt <= datetime.utcnow():
                ...
                normalized_doc = _normalize_store_status_doc(doc)
                await store_status_broadcaster.broadcast(_serialize_store_status(StoreStatus(**normalized_doc)))
    except Exception:
        pass
